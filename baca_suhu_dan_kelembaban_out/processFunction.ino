...
void proses(){
  ...
  // MENETAPKAN KODE MODUL DAN KODE VARIABEL UNTUK SUHU OUTDOOR
  suhuData.kodeModul = "07";
  suhuData.kodeVariabel = 11;
  // suhuData.data = avgTemp;

  // MENETAPKAN KODE MODUL DAN KODE VARIABEL UNTUK KELEMBABAN OUTDOOR
  kelembabanData.kodeModul = "07";
...
void kondisi(){
  // VARIABEL ALARM UNTUK SUHU OUTDOOR
  int LL_S = 15;
  int L_S = 24;
  int N_S = 32;
  int H_S = 37;

  // VARIABEL ALARM UNTUK KELEMBABAN OUTDOOR
  int LL_K = 40;
  int L_K = 60;
  int N_K = 85;
  int H_K = 95;

  // KONDISI kodeAlarm SUHU OUTDOOR
  if(suhuData.data < LL_S){
    suhuData.kodeAlarm = 1;
    suhuData.berita = "Luar Greenhouse Terlalu Dingin";
  }else if(suhuData.data >= LL_S && suhuData.data < L_S){
    suhuData.kodeAlarm = 2;
    suhuData.berita = "Luar Greenhouse Dingin";
  }else if(suhuData.data >= L_S && suhuData.data < N_S){
    suhuData.kodeAlarm = 0;
    suhuData.berita = "Luar Greenhouse Optimal";
  }else if(suhuData.data >= N_S && suhuData.data < H_S){
    suhuData.kodeAlarm = 3;
    suhuData.berita = "Luar Greenhouse Panas";
  }else{
    suhuData.kodeAlarm = 4;
    suhuData.berita = "Luar Greenhouse Terlalu Panas";
  }

  // KONDISI kodeAlarm KELEMBABAN OUTDOOR
  if(kelembabanData.data < LL_K){
    kelembabanData.kodeAlarm = 1;
    kelembabanData.berita = "Luar Greenhouse Terlalu Kering";
  }else if(kelembabanData.data >= LL_K && kelembabanData.data < L_K){
    kelembabanData.kodeAlarm = 2;
    kelembabanData.berita = "Luar Greenhouse Kering";
  }else if(kelembabanData.data >= L_K && kelembabanData.data < N_K){
    kelembabanData.kodeAlarm = 0;
    kelembabanData.berita = "Luar Greenhouse Optimal";
  }else if(kelembabanData.data >= N_K && kelembabanData.data < H_K){
    kelembabanData.kodeAlarm = 3;
    kelembabanData.berita = "Luar Greenhouse Lembab";
  }else{
    kelembabanData.kodeAlarm = 4;
    kelembabanData.berita = "Luar Greenhouse Terlalu Lembab";
  }
}
